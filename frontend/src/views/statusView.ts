...
export function renderStatusView(container: HTMLElement, transactionId: string): void {
  ...
  function render(tx: Transaction) {
    // Prefer per-amount asset fields; fall back to the legacy single field
    const debitCode    = tx.debitAssetCode   ?? tx.assetCode;
    const debitScale   = tx.debitAssetScale  ?? tx.assetScale;
    ...
    if (tx.status === 'COMPLETED') {
          ...
          <div class="quote-summary">
            ${tx.recipientName ? `
              <div class="summary-row">
                ...
                <span class="value">${tx.recipientName}</span>
              </div>
            ` : ''}
            <div class="summary-row">
              <span class="label">You sent</span>
              <span class="value">${fmt(tx.debitAmount, debitCode, debitScale)}</span>
            </div>
            <div class="summary-row">
      ...
      content.innerHTML = `
        <div class="status-terminal">
          ...
          <div class="error-msg">${tx.errorMessage ?? 'An unknown error occurred.'}</div>
